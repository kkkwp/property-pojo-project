...
public class MainView {
    ...
    private PropertyType selectPropertyTypeForRegistration() {
        ...
        switch (choice) {
            case "1": return PropertyType.APARTMENT;
            case "2": return PropertyType.VILLA;
            case "3": return PropertyType.OFFICETEL;
            case "4": return PropertyType.ONE_ROOM;
            default:
    ...
    private Location selectLocationForRegistration() {
        ...
        switch (choice) {
            case "1": return new Location("서울특별시", "강남구");
            case "2": return new Location("서울특별시", "서초구");
            case "3": return new Location("서울특별시", "마포구");
            case "4": return new Location("경기도", "수원시");
            case "5": return new Location("경기도", "성남시");
            default:
                System.out.println("❌ 잘못된 선택입니다.");
                System.out.print("계속하려면 Enter를 누르세요: ");
                scanner.nextLine();
    ...
    private DealType selectDealTypeForRegistration() {
        ...
        switch (choice) {
            case "1": return DealType.JEONSE;
            case "2": return DealType.MONTHLY;
            case "3": return DealType.SALE;
            default:
                System.out.println("❌ 잘못된 선택입니다.");
                System.out.print("계속하려면 Enter를 누르세요: ");
                scanner.nextLine();
    ...
    private Price inputPriceForRegistration(DealType dealType) {
        ...
        if (dealType == DealType.MONTHLY) {
            ...
            if (depositStr.equals("0")) return null;
            
            System.out.print("\u001B[33m월세 (원): \u001B[0m");
            String monthlyStr = scanner.nextLine().trim();
            if (monthlyStr.equals("0")) return null;
            ...
            if (priceStr.equals("0")) return null;
    ...
    private void updateDealType(User lessor, Property property) {
        ...
        switch (choice) {
            case "1": newDealType = DealType.JEONSE; break;
            case "2": newDealType = DealType.MONTHLY; break;
            case "3": newDealType = DealType.SALE; break;
            default:
                System.out.println("❌ 잘못된 선택입니다.");
                System.out.print("계속하려면 Enter를 누르세요: ");
                scanner.nextLine();
    ...
    private void updatePrice(User lessor, Property property) {
        ...
        if (property.getDealType() == DealType.MONTHLY) {
            System.out.print("\u001B[33m보증금 (원): \u001B[0m");
            String depositStr = scanner.nextLine().trim();
            if (depositStr.equals("0")) return;
            
            System.out.print("\u001B[33m월세 (원): \u001B[0m");
            String monthlyStr = scanner.nextLine().trim();
            if (monthlyStr.equals("0")) return;
        ...
        try {
            ...
            if (propertyIndex >= 0 && propertyIndex < myProperties.size()) {
                ...
                String confirmContent = "매물 삭제 확인\n\n" +
                                       ...
                                       "📍 위치: " + propertyToDelete.getLocation().getCity() + " " + propertyToDelete.getLocation().getDistrict() + "\n" +
    ...
    private void viewContractRequests(User lessor) {
        ...
        for (int i = 0; i < allRequests.size(); i++) {
            ...
            String statusText = "";
            switch (request.getStatus()) {
                case APPROVED: statusText = "승인 완료"; break;
                case REJECTED: statusText = "반려 완료"; break;
                case REQUESTED: statusText = "승인 대기 중"; break;
            }
