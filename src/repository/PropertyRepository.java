...
public class PropertyRepository {
	...
	public Property save(Property property) {
		...
		if (property.getId() == 0L) {
			// 새로운 매물 등록 - 새로운 ID 생성
			savedProperty = new Property(++sequence, property.getOwnerId(), property.getLocation(),
				property.getPrice(), property.getPropertyType(), property.getDealType());
		} else {
			// 기존 매물 수정 - 기존 ID 유지
			savedProperty = new Property(property.getId(), property.getOwnerId(), property.getLocation(),
				property.getPrice(), property.getPropertyType(), property.getDealType());
		}
		
		properties.put(savedProperty.getId(), savedProperty);
